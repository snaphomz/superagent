...
import { questionGenerator } from './src/ai/questionGenerator.js';
...
async function processRecentUpdates(hoursBack = 24) {
  ...
  try {
    ...
    for (const update of eodUpdates.reverse()) {
      ...
      console.log(`   Preview: ${update.text.substring(0, 100)}...`);
      ...
      if (analysis.issues.length > 0) {
        ...
        try {
          const result = await responseGenerator.generateResponse(update);
          
          console.log(`   ✅ Generated: "${result.response.substring(0, 80)}..."`);
          console.log(`   Confidence: ${result.confidence.toFixed(1)}%`);
          
          const shouldSend = await askUserConfirmation(
            `\nSend this response to ${userName}? (y/n): `
          );
    ...
    console.log(`\n✅ Processed ${eodUpdates.length} EOD updates\n`);
...
processRecentUpdates(hoursBack).then(() => process.exit(0));
