...
import { createMinimalSlackBot } from './src/bot/minimalSlackBot.js';
...
async function startSlackBot() {
  try {
    console.log('🤖 Starting minimal Slack bot...');
    const app = createMinimalSlackBot();
    
    await app.start();
    console.log('✅ Slack bot is connected and monitoring messages!');
    console.log(`👀 Monitoring channel ${config.target.channelId} for EOD updates...`);
    
  } catch (error) {
    console.error('❌ Failed to start Slack bot:', error);
    // Continue running HTTP server even if Slack bot fails
  }
}
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  // Start Slack bot after HTTP server is ready
  startSlackBot();
});

console.log('🚀 Hybrid server starting (HTTP + Slack bot)...');
